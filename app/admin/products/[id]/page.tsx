        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-mf-gray mt-1">Product Details & Analytics</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <Link 
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {product.images.length > 0 ? (
              ...
              <div className="aspect-square bg-mf-light-gray rounded-lg flex items-center justify-center">
                <Package className="w-16 h-16 text-mf-gray" />
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="text-mf-gray mb-4">{product.description}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div>
                <p className="text-sm text-mf-gray mb-1">Price</p>
                <p className="text-2xl font-bold">${product.price}</p>
                {product.compareAtPrice && (
                  <p className="text-sm text-mf-gray line-through">${product.compareAtPrice}</p>
                )}
              </div>
              <div>
                <p className="text-sm text-mf-gray mb-1">Category</p>
                <p className="font-semibold">{product.category.replace('_', ' ')}</p>
              </div>
              <div>
                <p className="text-sm text-mf-gray mb-1">Stock</p>
            ...
            {product.tags.length > 0 && (
              <div className="mb-6">
                <p className="text-sm text-mf-gray mb-2">Tags</p>
                <div className="flex flex-wrap gap-2">
                  {product.tags.map((tag, index) => (
                    <span key={index} className="bg-mf-light-gray px-3 py-1 rounded-full text-sm">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">${product.salesAnalytics?.totalRevenue || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm text-mf-gray">Units Sold</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm text-mf-gray">Views</p>
              <p className="text-2xl font-bold">{product.salesAnalytics?.viewCount || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm text-mf-gray">Rating</p>
              <p className="text-2xl font-bold">{product.avgRating.toFixed(1)}</p>
            ...
            {product.variants.map((variant) => (
              <div key={variant.id} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm bg-mf-light-gray px-2 py-1 rounded">{variant.type}</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {variant.options.map((option, index) => (
                    <span key={index} className="text-sm bg-white border border-gray-300 px-2 py-1 rounded">
                      {option}
                    </span>
                  ))}
                </div>
